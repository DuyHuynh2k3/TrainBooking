...
function HomeBlogPage() {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        let apiUrl = "http://localhost:3000/api/blogs";
  ...
  if (loading) return <div className="text-center py-5"><h2>Đang tải...</h2></div>;
          ...
          {currentBlogs.length > 0 ? (
            <div className="row justify-content-center p-0">
              {currentBlogs.map((blog) => (
                <div key={blog.id} className="col-12 col-md-6 col-lg-4 d-flex justify-content-center mb-4">
                  <BlogCard blog={blog} />
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-5">
              <h4>{category ? `Không có bài viết nào trong mục ${displayCategory}` : "Không có bài viết nào"}</h4>
            </div>
          )}
        </div>
      </main>
